...
const toCssLength = (value: string | number | undefined | null): string | undefined =>
...
const useResizeObserver = (
  ...
  useEffect(() => {
    ...
    const observers = elements.map(ref => {
      if (!ref.current) return null as null;
...
const useImageLoader = (
  ...
  useEffect(() => {
    ...
    return () => {
      images.forEach(img => {
        img.removeEventListener('load', handleImageLoad as any);
        img.removeEventListener('error', handleImageLoad as any);
      });
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    if (seqSize > 0) {
      ...
      (track as HTMLElement).style.transform = transformValue;
    }

    if (prefersReduced) {
      (track as HTMLElement).style.transform = isVertical ? 'translate3d(0, 0, 0)' : 'translate3d(0, 0, 0)';
      return () => {
